...
/*
把ORBSLAM2里的特征提取器挖出来放到自己的工程里却发现每次匹配的结果都不一样，
导致如果出现其他错误排查起来很困难。仔细检查才发现是特征提取器里对节点的排序
会受到cv::imshow函数的影响，进而影响了特征匹配的结果。工程在下面的地址，对于
同一张图每次运行结果都有所不同。程序输出了提取的特征点坐标，运行两次就会发现
这两次提取的特征点结果是不一样的。
*/
